...
public static class ArchiveEncryption
{
    ...
    private static bool Unwrap(Stream input, Stream output)
    {
        ...
        const int offsetToSeek = 0x10000;
            
        if (input.Length < offsetToSeek)
        {
            input.Position = basePosition;
            return false;
        }
            
        input.Seek(offsetToSeek, SeekOrigin.Begin);
        ...
        foreach (ArchiveEncryptionSettings candidate in Settings)
        {
            ...
            uint computedHash = Fnv32.Hash(headerBytes, 0, 12);
