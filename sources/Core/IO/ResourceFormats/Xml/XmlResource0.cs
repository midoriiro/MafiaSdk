...
using Core.IO.ResourceFormats.Xml.Xml0;
...
namespace Core.IO.ResourceFormats.Xml
{
    public class XmlResource0
    {
        public static void Serialize(Stream stream, string content, Endian endian)
        {
            ...
            stream.WriteValueS32(nodes.Count, endian);
            stream.WriteValueU32((uint)valueData.Length, endian);
            stream.WriteFromStream(valueData, (uint)valueData.Length);
            stream.WriteFromStream(nodeData, nodeData.Length);
        }

        public static string Deserialize(Stream stream, Endian endian)
        {
            if (stream.ReadValueU8() != 4)
            {
            ...
            uint dataSize = stream.ReadValueU32(endian);
            ...
            MemoryStream data = stream.ReadToMemoryStream((int)dataSize);
            ...
            for (uint i = 0; i < count; i++)
            {
                ...
                {
                    Name = DataValue.Deserialize(data, stream.ReadValueU32(endian), endian)!,
                    Value = DataValue.Deserialize(data, stream.ReadValueU32(endian), endian),
                    Id = stream.ReadValueU32(endian)
                };

                uint childCount = stream.ReadValueU32(endian);
                node.Children.Clear();
                
                for (uint j = 0; j < childCount; j++)
                {
                    node.Children.Add(stream.ReadValueU32(endian));
                }

                uint attributeCount = stream.ReadValueU32(endian);
